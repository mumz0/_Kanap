...
function DisplayTotalCart() {
    if (itemsList.length == 0) {
        ...
        const totalQuantity = new Intl.NumberFormat('fr-FR').format(productObjList.reduce((accumulator, currentItem) => accumulator + currentItem.quantity, 0));
        const totalPrice = new Intl.NumberFormat('fr-FR').format(productObjList.reduce((accumulator, currentItem) => accumulator + currentItem.totalPrice, 0));
...
function updateItem(event) {
    ...
    let itemCart = itemsList.find(a => a.id === product.dataset.id && a.color === product.dataset.color)
    let itemObj = productObjList.find(a => a.id === product.dataset.id && a.color === product.dataset.color)
    if (itemCart && itemObj) {
...
function deleteItemInCart(event) {
    ...
    const index = itemsList.findIndex(a => a.id === itemToDelete.dataset.id && a.color === itemToDelete.dataset.color);
    const indexObj = productObjList.findIndex(a => a.id === itemToDelete.dataset.id && a.color === itemToDelete.dataset.color);
    if (index > -1) {
